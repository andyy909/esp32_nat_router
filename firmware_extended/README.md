...
These verified builds contain the extended live dashboard and access-mode
features. They were built with ESP-IDF 5.4.1.
...
esptool --chip esp32s3 --port COM6 --baud 460800 write-flash `
  0x20000 firmware_extended/esp32s3_n16r8/esp32_nat_router.bin
```
...
```powershell
esptool --chip esp32s3 --port COM6 --baud 460800 write-flash `
  0x0     firmware_extended/esp32s3_n16r8/bootloader.bin `
  0x8000  firmware_extended/esp32s3_n16r8/partition-table.bin `
  0xf000  firmware_extended/esp32s3_n16r8/ota_data_initial.bin `
  0x20000 firmware_extended/esp32s3_n16r8/esp32_nat_router.bin
```

## Classic ESP32-WROOM-32

For an application-only update of a device already using this mod's partition
layout:

```powershell
esptool --chip esp32 --port COM5 --baud 460800 write-flash `
  0x20000 firmware_extended/esp32_wroom32/esp32_nat_router.bin
```
...
```powershell
esptool --chip esp32 --port COM5 --baud 460800 write-flash `
  0x1000  firmware_extended/esp32_wroom32/bootloader.bin `
  0x8000  firmware_extended/esp32_wroom32/partition-table.bin `
  0xf000  firmware_extended/esp32_wroom32/ota_data_initial.bin `
  0x20000 firmware_extended/esp32_wroom32/esp32_nat_router.bin
```
...
build completed successfully and passed image and partition-size validation.
