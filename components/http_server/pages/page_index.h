...
body{font-family:Arial,sans-serif;background:#111827;color:#e5e7eb;padding:1rem;min-height:100vh;line-height:1.5}\
#container{max-width:880px;margin:auto;padding:1.25rem;background:var(--panel);border-radius:18px;border:1px solid rgba(0,217,255,.09)}\
...
h1{font-size:1.55rem;font-weight:650;color:var(--cyan);line-height:1.15}\
...
.card{min-height:112px;padding:.9rem;background:var(--card);border:1px solid rgba(0,217,255,.11);border-radius:13px}\
...
.nav-button{background:#667eea;color:#fff;border:0;border-radius:11px;padding:.92rem .7rem;font-size:.86rem;font-weight:650;text-decoration:none;display:flex;align-items:center;justify-content:center;text-align:center}\
...
setText('live-uplink',connected?'Online':'Offline');setClass('live-uplink',connected?'good':'bad');setText('live-uplink-sub',connected?(ip&&ip!=='N/A'?ip:'Connected'):'No Internet uplink');\
setText('live-clients',clients||'0');setText('live-clients-sub',(clients==='1'?'Device connected':'Devices connected'));\
setText('live-uptime',uptime?uptime.split(' (')[0]:'—');setText('live-uptime-sub',uptime.indexOf('since ')>=0?uptime.split('since ')[1].replace(')',''):'since last boot');\
setText('live-signal',rssi===null?'—':rssi+' dBm');setClass('live-signal',rssi===null?'':rssi<-78?'bad':rssi<-68?'warn':'good');setText('live-signal-sub',rssi===null?'no reading':Math.round(q)+'% quality');signalBars(q);\
var b=exactBytes(doc)||parseBytes(bytes),now=Date.now();if(b){setText('live-total',humanBytes(b.rx));setText('live-total-sub','Upload '+humanBytes(b.tx));}else{setText('live-total','—');setText('live-total-sub','no counters');}if(b&&lastBytes&&now>lastTime){var sec=(now-lastTime)/1000;setText('live-speed',speed((b.rx-lastBytes.rx)/sec));setText('live-speed-sub','Upload ↑ '+speed((b.tx-lastBytes.tx)/sec));}else{setText('live-speed',b?'calculating':'—');setText('live-speed-sub',b?'Reading after 5 seconds':'no counters');}if(b){lastBytes=b;lastTime=now;}\
var warnings=[];if(!connected)warnings.push('Uplink disconnected.');if(rssi!==null&&rssi<-78)warnings.push('Weak uplink signal ('+rssi+' dBm).');if(doc.body.textContent.indexOf('No Password Protection')>=0)warnings.push('Web UI has no password.');var box=document.getElementById('dashboard-warning');if(box){box.textContent=warnings.join(' ');box.className=warnings.length?'warning-box show':'warning-box';}\
var live=document.getElementById('live-dot');if(live)live.className=connected?'live-dot':'live-dot offline';setText('last-update','Updated '+new Date().toLocaleTimeString());\
}\
...
function statusPoll(manual){if(document.hidden&&!manual){scheduleStatus();return;}request('/',function(html){var d=new DOMParser().parseFromString(html,'text/html');updateFromDoc(d);scheduleStatus();},function(){var dot=document.getElementById('live-dot');if(dot)dot.className='live-dot offline';setText('last-update','Update failed');scheduleStatus();});}\
...
function renderClients(doc){var table=clientTableFromDoc(doc),body=document.getElementById('client-body'),note=document.getElementById('clients-note');if(!body)return;body.innerHTML='';if(!table){body.innerHTML=\"<tr><td colspan='5' class='client-empty'>Sign in to view clients.</td></tr>\";if(note)note.textContent='protected';return;}var heads=table.querySelectorAll('th'),hasTraffic=false;for(var h=0;h<heads.length;h++)if(/Traffic/i.test(text(heads[h])))hasTraffic=true;var rows=table.querySelectorAll('tbody tr'),count=0;for(var i=0;i<rows.length;i++){var c=rows[i].querySelectorAll('td');if(c.length<3||/No connected/i.test(text(rows[i])))continue;var mac=text(c[0]),ip=text(c[1]),name=text(c[2])||'Unknown',traffic=hasTraffic&&c.length>3?text(c[3]):'Stats off';var tr=document.createElement('tr');tr.innerHTML=\"<td><div class='device-name'></div></td><td></td><td></td><td></td><td><a class='manage-link' href='/mappings'>Manage</a></td>\";tr.querySelector('.device-name').textContent=name;tr.children[1].textContent=ip;tr.children[2].textContent=mac;tr.children[3].textContent=traffic;body.appendChild(tr);count++;}if(!count)body.innerHTML=\"<tr><td colspan='5' class='client-empty'>No devices connected.</td></tr>\";if(note)note.textContent=count+' online';}\
function clientsPoll(manual){if(document.hidden&&!manual){scheduleClients();return;}request('/mappings',function(html,url){var d=new DOMParser().parseFromString(html,'text/html');renderClients(d,url);scheduleClients();},function(){setText('clients-note','unavailable');scheduleClients();});}\
...
<div class='livebar'>\
<div class='live-state'><span id='live-dot' class='live-dot'></span><span id='last-update'>Loading live data</span></div>\
<button class='refresh-button' type='button' onclick='liveRefresh()'>Refresh Now</button>\
</div>\
<div id='dashboard-warning' class='warning-box'></div>\
<div class='cards'>\
<div class='card'><div class='card-label'>Uplink</div><div id='live-uplink' class='card-value'>—</div><div id='live-uplink-sub' class='card-sub'>Connection status</div></div>\
<div class='card'><div class='card-label'>Signal</div><div id='live-signal' class='card-value'>—</div><div id='live-signal-sub' class='card-sub'>Uplink quality</div><div id='signal-bars' class='signal'><i></i><i></i><i></i><i></i></div></div>\
<div class='card'><div class='card-label'>Clients</div><div id='live-clients' class='card-value'>—</div><div id='live-clients-sub' class='card-sub'>Devices connected</div></div>\
<div class='card'><div class='card-label'>Current Download</div><div id='live-speed' class='card-value'>—</div><div id='live-speed-sub' class='card-sub'>Calculating data rate</div></div>\
<div class='card'><div class='card-label'>Total Download</div><div id='live-total' class='card-value'>—</div><div id='live-total-sub' class='card-sub'>Total upload</div></div>\
<div class='card'><div class='card-label'>Uptime</div><div id='live-uptime' class='card-value'>—</div><div id='live-uptime-sub' class='card-sub'>since last boot</div></div>\
</div>\
...
<div class='clients-head'><div class='clients-title'>Connected Devices</div><div id='clients-note' class='clients-note'>loading</div></div>\
<div class='client-wrap'>\
<table class='client-table'>\
<thead><tr><th>Device</th><th>IP</th><th>MAC</th><th>Traffic</th><th></th></tr></thead>\
<tbody id='client-body'><tr><td colspan='5' class='client-empty'>Loading device list…</td></tr></tbody>\
</table>\
</div>\
</div>"
...
<a href='/setup' class='nav-button'>Quick Setup</a>\
<a href='/scan' class='nav-button'>Wi-Fi Scan</a>\
<a href='/config' class='nav-button'>Configuration</a>\
<a href='/mappings' class='nav-button'>Clients &amp; Mappings</a>\
<a href='/access' class='nav-button'>Access Mode</a>\
